...
from flaxlight.cmds.init_funcs import flaxlight_init
from flaxlight.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from flaxlight.daemon.keychain_server import KeychainServer, keychain_commands
from flaxlight.daemon.windows_signal import kill
from flaxlight.server.server import ssl_context_for_root, ssl_context_for_server
from flaxlight.ssl.create_ssl import get_mozilla_ca_crt
...
from flaxlight.util.keychain import (
    Keychain,
...
from flaxlight.util.service_groups import validate_service
from flaxlight.util.setproctitle import setproctitle
from flaxlight.util.ws_message import WsRpcMessage, create_payload, format_response
...
service_plotter = "flaxlight plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "flaxlight": "flaxlight",
        "flaxlight_wallet": "start_wallet",
        "flaxlight_full_node": "start_full_node",
        "flaxlight_harvester": "start_harvester",
        "flaxlight_farmer": "start_farmer",
        "flaxlight_introducer": "start_introducer",
        "flaxlight_timelord": "start_timelord",
        "flaxlight_timelord_launcher": "timelord_launcher",
        "flaxlight_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("flaxlight daemon exiting in 5 seconds")
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "flaxlight_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in flaxlight_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    flaxlight_init(root_path, should_check_keys=False)
    config = load_config(root_path, "config.yaml")
    setproctitle("flaxlight_daemon")
