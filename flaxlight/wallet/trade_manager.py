...
from flaxlight.protocols.wallet_protocol import CoinState
...
from flaxlight.wallet.cc_wallet import cc_utils
...
from flaxlight.wallet.trade_record import TradeRecord
from flaxlight.wallet.trading.trade_status import TradeStatus
from flaxlight.wallet.trading.trade_store import TradeStore
from flaxlight.wallet.transaction_record import TransactionRecord
from flaxlight.wallet.util.trade_utils import (
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_flaxlight(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_spends:
            ...
            if matched:
                ...
                # standard flaxlight coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if flaxlight_discrepancy is None:
                    flaxlight_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    flaxlight_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        flaxlight_spend_bundle: Optional[SpendBundle] = None
        if flaxlight_discrepancy is not None:
            flaxlight_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_flaxlight(
                flaxlight_discrepancy, []
            )
            if flaxlight_spend_bundle is not None:
                for coinsol in coinsols:
                    flaxlight_spend_bundle.coin_spends.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if flaxlight_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = flaxlight_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have flaxlight available",
                        )
        ...
        now = uint64(int(time.time()))
        if flaxlight_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, flaxlight_spend_bundle])
            if flaxlight_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(flaxlight_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=flaxlight_spend_bundle,
                    additions=flaxlight_spend_bundle.additions(),
                    removals=flaxlight_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=flaxlight_spend_bundle.name(),
                    memos=list(flaxlight_spend_bundle.get_memos().items()),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
