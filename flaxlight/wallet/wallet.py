...
from flaxlight.wallet.puzzles.puzzle_utils import (
...
from flaxlight.wallet.secret_key_store import SecretKeyStore
from flaxlight.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_flaxlight(self, flaxlight_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if flaxlight_amount < 0:
            utxos = await self.select_coins(abs(flaxlight_amount), exclude)
        else:
        ...
        flaxlight_amount = spend_value + flaxlight_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": flaxlight_amount}]
