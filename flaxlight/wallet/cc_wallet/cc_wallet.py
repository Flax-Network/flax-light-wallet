...
from flaxlight.protocols.wallet_protocol import PuzzleSolutionResponse, CoinState
...
from flaxlight.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from flaxlight.util.ints import uint8, uint32, uint64, uint128
from flaxlight.util.json_util import dict_to_json_str
from flaxlight.wallet.cc_wallet.cat_constants import DEFAULT_CATS
from flaxlight.wallet.cc_wallet.cc_info import CCInfo
...
from flaxlight.wallet.derivation_record import DerivationRecord
from flaxlight.wallet.lineage_proof import LineageProof
from flaxlight.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
from flaxlight.wallet.wallet_coin_record import WalletCoinRecord
from flaxlight.wallet.wallet_info import WalletInfo
...
class CCWallet:
    ...
    async def create_new_cc_wallet(
        ...
        try:
            flaxlight_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        flaxlight_tx = replace(flaxlight_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(flaxlight_tx)
    ...
    async def create_tandem_xfx_tx(
        ...
        if fee > amount_to_claim:
            flaxlight_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(flaxlight_coins)[0].name()
            selected_amount = sum([c.amount for c in flaxlight_coins])
            flaxlight_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
                (await self.standard_wallet.get_new_puzzlehash()),
                fee=uint64(fee - amount_to_claim),
                coins=flaxlight_coins,
            ...
            assert flaxlight_tx.spend_bundle is not None

            message = None
            for spend in flaxlight_tx.spend_bundle.coin_spends:
        ...
        return flaxlight_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        regular_flaxlight_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_flaxlight_to_claim = payment_amount

        need_flaxlight_transaction = (fee > 0 or regular_flaxlight_to_claim > 0) and (fee - regular_flaxlight_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                first = False
                if need_flaxlight_transaction:
                    if fee > regular_flaxlight_to_claim:
                        announcement = Announcement(coin.name(), b"$", b"\xca")
                        flaxlight_tx, _ = await self.create_tandem_xfx_tx(
                            fee, uint64(regular_flaxlight_to_claim), announcement_to_assert=announcement
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries, coin_announcements={announcement.message}
                        )
                    elif regular_flaxlight_to_claim > fee:
                        flaxlight_tx, _ = await self.create_tandem_xfx_tx(fee, uint64(regular_flaxlight_to_claim))
                        innersol = self.standard_wallet.make_solution(
        ...
        flaxlight_spend_bundle = SpendBundle([], G2Element())
        if flaxlight_tx is not None and flaxlight_tx.spend_bundle is not None:
            flaxlight_spend_bundle = flaxlight_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    flaxlight_spend_bundle,
                ]
            ),
            flaxlight_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, flaxlight_tx = await self.generate_unsigned_spendbundle(payments, fee, coins=coins)
        ...
        if flaxlight_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=flaxlight_tx.confirmed_at_height,
                    created_at_time=flaxlight_tx.created_at_time,
                    to_puzzle_hash=flaxlight_tx.to_puzzle_hash,
                    amount=flaxlight_tx.amount,
                    fee_amount=flaxlight_tx.fee_amount,
                    confirmed=flaxlight_tx.confirmed,
                    sent=flaxlight_tx.sent,
                    spend_bundle=None,
                    additions=flaxlight_tx.additions,
                    removals=flaxlight_tx.removals,
                    wallet_id=flaxlight_tx.wallet_id,
                    sent_to=flaxlight_tx.sent_to,
                    trade_id=flaxlight_tx.trade_id,
                    type=flaxlight_tx.type,
                    name=flaxlight_tx.name,
                    memos=[],
                )
            )
