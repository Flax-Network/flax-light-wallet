...
def make_puzzle(amount: int) -> int:
    ...
    if error:
        ...
        for cvp in result_human:
            ...
            total_flaxlight += int_from_bytes(cvp.vars[1])
            print(
                f"{ConditionOpcode(cvp.opcode).name}: {encode_puzzle_hash(cvp.vars[0], prefix)},"
                f" amount: {int_from_bytes(cvp.vars[1])}"
            )
    return total_flaxlight


total_flaxlight = 0
print("Pool address: ")
total_flaxlight += make_puzzle(pool_amounts)
print("\nFarmer address: ")
total_flaxlight += make_puzzle(farmer_amounts)

assert total_flaxlight == calculate_base_farmer_reward(uint32(0)) + calculate_pool_reward(uint32(0))
