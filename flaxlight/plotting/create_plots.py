...
from flaxlight.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from flaxlight.plotting.util import add_plot_directory
from flaxlight.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
from flaxlight.util.config import config_path_for_filename, load_config
from flaxlight.util.keychain import Keychain
from flaxlight.util.path import mkdir
from flaxlight.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'flaxlight keys add', 'flaxlight keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'flaxlight keys add', 'flaxlight keys generate' or provide a public key with -p"
            )
