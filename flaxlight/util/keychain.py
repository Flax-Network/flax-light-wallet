...
DEFAULT_USER = f"user-flax-{CURRENT_KEY_VERSION}"  # e.g. user-flaxlight-1.8
DEFAULT_SERVICE = f"flax-{DEFAULT_USER}"  # e.g. flaxlight-user-flaxlight-1.8
...
def supports_keyring_passphrase() -> bool:
    return False or os.environ.get("FLAX_PASSPHRASE_SUPPORT", "").lower() in ["1", "true"]
...
def obtain_current_passphrase(prompt: str = DEFAULT_PASSPHRASE_PROMPT, use_passphrase_cache: bool = False) -> str:
    ...
    before failing.
    """
    from flaxlight.cmds.passphrase_funcs import prompt_for_passphrase
...
class Keychain:
    ...
    def migrate_legacy_keyring(
        ...
        """
        raise Exception('Keyring migration is not supported by the Flax Light Wallet.')
