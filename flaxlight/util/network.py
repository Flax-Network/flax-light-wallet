...
def class_for_type(type: NodeType) -> Any:
    if type is NodeType.FULL_NODE:
        ...
        from flaxlight.wallet.wallet_node_api import WalletNodeAPI

        return WalletNodeAPI
    elif type is NodeType.INTRODUCER:
        from flaxlight.introducer.introducer_api import IntroducerAPI

        return IntroducerAPI
    elif type is NodeType.TIMELORD:
        from flaxlight.timelord.timelord_api import TimelordAPI

        return TimelordAPI
    elif type is NodeType.FARMER:
        from flaxlight.farmer.farmer_api import FarmerAPI

        return FarmerAPI
    elif type is NodeType.HARVESTER:
        from flaxlight.harvester.harvester_api import HarvesterAPI
