...
from flaxlight.util.default_root import DEFAULT_KEYS_ROOT_PATH
from flaxlight.util.file_keyring import FileKeyring
from flaxlight.util.misc import prompt_yes_no
...
def check_legacy_keyring_keys_present(keyring: Union[MacKeyring, WinKeyring]) -> bool:
    ...
    from flaxlight.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _configure_backend(self) -> Union[LegacyKeyring, FileKeyring]:
        from flaxlight.util.keychain import supports_keyring_passphrase
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from flaxlight.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'flaxlight passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from flaxlight.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'flaxlight passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from flaxlight.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from flaxlight.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
