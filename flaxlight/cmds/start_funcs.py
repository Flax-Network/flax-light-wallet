...
from flaxlight.cmds.passphrase_funcs import get_current_passphrase
from flaxlight.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from flaxlight.util.keychain import KeyringMaxUnlockAttempts
from flaxlight.util.service_groups import services_for_groups
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    flaxlight = sys.argv[0]
    process = subprocess.Popen(f"{flaxlight} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the flaxlight daemon")
        return None
