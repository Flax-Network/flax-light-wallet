...
from flaxlight import __version__
from flaxlight.consensus.coinbase import create_puzzlehash_for_pk
from flaxlight.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_flaxlight_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from flaxlight.util.bech32m import encode_puzzle_hash
from flaxlight.util.config import (
    create_default_flaxlight_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from flaxlight.util.keychain import Keychain
from flaxlight.util.path import mkdir
from flaxlight.util.ssl_check import (
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'flaxlight keys generate'")
        return None
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-2] == config["farmer"].get("xfx_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-2] == config["pool"].get("xfx_target_address"):
                stop_searching_for_pool = True
...
def create_all_ssl(root_path: Path):
    ...
    flaxlight_ca_crt, flaxlight_ca_key = get_flaxlight_ca_crt_key()
    flaxlight_ca_crt_path = ca_dir / "flaxlight_ca.crt"
    flaxlight_ca_key_path = ca_dir / "flaxlight_ca.key"
    write_ssl_cert_and_key(flaxlight_ca_crt_path, flaxlight_ca_crt, flaxlight_ca_key_path, flaxlight_ca_key)
    ...
    flaxlight_ca_crt, flaxlight_ca_key = get_flaxlight_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, flaxlight_ca_crt, flaxlight_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if flaxlight_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return flaxlight_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def flaxlight_version_number() -> Tuple[str, str, str, str]:
...
def flaxlight_minor_release_number():
    res = int(flaxlight_version_number()[2])
    print(f"Install release number: {res}")
    return res


def flaxlight_full_version_str() -> str:
    major, minor, patch, dev = flaxlight_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def flaxlight_init(root_path: Path, *, should_check_keys: bool = False, fix_ssl_permissions: bool = False):
    """
    ...
    if os.environ.get("FLAX_ROOT", None) is not None:
        print(
            ...
            f"Please unset the environment variable and run flaxlight init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Flax directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if FLAX_ROOT is set, or if user has run flaxlight init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'flaxlight keys show --show-mnemonic-seed'")
