...
from flaxlight.cmds.configure import configure_cmd
from flaxlight.cmds.farm import farm_cmd
from flaxlight.cmds.init import init_cmd
from flaxlight.cmds.keys import keys_cmd
from flaxlight.cmds.netspace import netspace_cmd
from flaxlight.cmds.passphrase import passphrase_cmd
from flaxlight.cmds.plots import plots_cmd
from flaxlight.cmds.show import show_cmd
from flaxlight.cmds.start import start_cmd
from flaxlight.cmds.stop import stop_cmd
from flaxlight.cmds.wallet import wallet_cmd
from flaxlight.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from flaxlight.util.keychain import set_keys_root_path, supports_keyring_passphrase
from flaxlight.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage flaxlight blockchain infrastructure ({__version__})\n",
    epilog="Try 'flaxlight start node', 'flaxlight netspace -d 192', or 'flaxlight show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from flaxlight.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from flaxlight.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show flaxlight version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs flaxlight daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by flaxlight start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from flaxlight.daemon.server import async_run_daemon
