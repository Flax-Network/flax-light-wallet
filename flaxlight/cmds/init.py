...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.flaxlight/[version]/config/ssl/ca
    - Shut down all flaxlight daemon processes with `flaxlight stop all -d`
    - Run `flaxlight init -c [directory]` on your remote harvester,
    ...
    from flaxlight.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import flaxlight_init
    from flaxlight.util.default_root import DEFAULT_ROOT_PATH

    flaxlight_init(DEFAULT_ROOT_PATH)
