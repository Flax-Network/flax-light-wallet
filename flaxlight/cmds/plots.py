...
def show_plots(root_path: Path):
    from flaxlight.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'flaxlight plots add -d [dir]' and remove with"
        + " 'flaxlight plots remove -d [dir]'"
        + " Scan and check plots with 'flaxlight plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'flaxlight init'")
...
def create_cmd(
    ...
    from flaxlight.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from flaxlight.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from flaxlight.plotting.util import remove_plot_directory
