...
from flaxlight.protocols.protocol_state_machine import message_requires_reply
from flaxlight.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from flaxlight.protocols.shared_protocol import protocol_version
from flaxlight.server.introducer_peers import IntroducerPeers
from flaxlight.server.outbound_message import Message, NodeType
from flaxlight.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from flaxlight.util.errors import Err, ProtocolError
from flaxlight.util.ints import uint16
from flaxlight.util.network import is_localhost, is_in_network
from flaxlight.util.ssl_check import verify_ssl_certs_and_keys
...
class FlaxServer:
    def __init__(
        ...
        flaxlight_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        ...
        self.flaxlight_ca_crt_path, self.flaxlight_ca_key_path = flaxlight_ca_crt_key
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.flaxlight_ca_crt_path, self.flaxlight_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.flaxlight_ca_crt_path, self.flaxlight_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSFlaxConnection] = None
        try:
