...
. ./activate
pip install ".[dev]"
black flaxlight tests && mypy flaxlight tests && flake8 flaxlight tests
...
7. Preferences > Settings > mypy > Targets: set to `./flaxlight` and `./tests`
...
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the flaxlight blockchain.
